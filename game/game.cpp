...
private:
    ...
    private:
        ...
        void oneMove(set<Position>* s, const Position& cur, const Position& dir, const bool& legalMove) {
            ...
            if (boundaryCheck(next)) {
                if (board[next.y][next.x].empty() or isEnemy(cur, next))
                    append(s, cur, next, legalMove);
                else if (isAlly(cur, next) && not legalMove)
                    append(s, cur, next, legalMove);
            }
        }
        ...
        set<Position>* get_legalMoveList(const Position& pos) {
            ...
            case PAWN: set<Position>* a = pawn_move(pos); s = pawn_attack(pos, true);
    ...
    public:
        ...
        void move(const Position& cur, const Position& dest, const int& promotion) {
        ...
        const bool castling_check(const Color& color, const bool& isKingside) {
            ...
            bool condition_2 = isKingside ? board[rank][5].empty() and board[rank][6].empty() :
                board[rank][3].empty() and board[rank][2].empty() and board[rank][1].empty();
            bool condition_3 = isKingside ? !board[rank][5].isAttacked(color) and !board[rank][6].isAttacked(color) :
                !board[rank][3].isAttacked(color) and !board[rank][2].isAttacked(color);
