...
// private: 
const bool Chess::boundaryCheck(const Position& pos) { return (pos.x < 0 || pos.x > 7 || pos.y < 0 || pos.y > 7) ? false : true; }
const bool Chess::isAlly(const Position& a, const Position& b) {
...
const bool Chess::isEnemy(const Position& a, const Position& b) {
    return ((board[a.y][a.x].piece.color != NOCOLOR && board[b.y][b.x].piece.color != NOCOLOR) &&
        board[a.y][a.x].piece.color != board[b.y][b.x].piece.color);
}
const bool Chess::isCheck(const Color& color) { return board[king_position_wb[color].y][king_position_wb[color].x].isAttacked(color); }
...
set<pair<Position, Position>>* Chess::get_candidateMove(const Color& color) {
